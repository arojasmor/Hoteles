...
1) Identificar y entender el comportamiento de los clientes en ambos hoteles.
2) Determinar si hay diferencias entre los patrones de ambos hoteles.
...
La información para este análisis es pública y los datos así como su descripcón se encuentran en la página de [ScienceDirect](https://www.sciencedirect.com/science/article/pii/S2352340918315191).

Ambos hoteles comparten la misma estructura de datos contenida en 32 variables y el hotel resort tiene 40,060 observaciones y el hotel de ciudad 79,330 observaciones. Cada registro representa una reservación.
...
* una segunda variable con 16340 valores NULL
* una tercer variable tiene 112593 valores NULL
...
Al observar la gráfica anterior podemos determinar las siguientes acciones a seguir:
...
Continuación variables tipo character:
...
* Variable company: eliminarla, ya que hay demasiados valores NULL (112593 de 119390).
* Para la variable deposit_type: juntar la categoría de menor frecuencia con la categoía
  Non Refund.
...
Finalmente, revisamos la unica variable continua que tenemos, adr:
...
descr(df$adr, 
      ...
      justify = "c",
      headings = T) %>% 
  kbl(digits = 4, align = "c") %>% kable_styling(full_width = F)
...
Con apoyo de las tablas resumen sabemos que es de 5400. Es una sola observación la que está afectado a toda la variable. Aquí lo recomendable será:

* Eliminar la observacion extrema de 5400.
...
count(df, children, sort = T) %>% # children
  kbl(align = "c") %>% kable_styling(full_width = F)
...
Como solo se identifico una observacion exrema, se procederá a eliminarla del dataset:
...
Ahora, lo siguiente es discretizar las tres variables que por su distribución muy dispersa es mejor modificarlas:
...
En las gráficas ya podemos apreciar mejor las distribuciones de las variables, pues no hay asimetrías fuertes ni desbalances severos en las variables de tipo factor. Las variables binarias o lógicas se ven bien y la única variable continua aunque presenta valores atípicos ya no hay presencia de valores extremos enmascarados. 

&nbsp;

## Creación de variables sintéticas

Una de las partes más interesantes en cualquier proyecto de discovery es el de crear variables nuevas a partir de las variables originales, las cuales son comúnmente llamadas variables sintéticas. Estas variables sintéticas deben de crearse con un sentido de negocio. A continuación, crearemos algunas variables que nos servirán para la etapa de discovery.
...
Podemos incluir otra variable para saber si la persona es extranjera o nacional, entendiendo que nacional correspondería al país Portugal.
...
Tambien, puede ser de utilidad conocer las noches totales que las personas se quedaron en algún hotel, confomadas por la suma de las noches entre semana más las noches en fin de semana.
...
- facturacion_reserva: lo que se ha facturado en esa reservación como resultado de multiplicar
  la variable creada noches_totales por adr
...
Técnicamente, para poder determinar si una serie presenta estacionalidad, entendida ésta como tendencias o patrones que se repiten constantemente dentro de un periodo de tiempo, es necesario al memos cuatro años de historia, sin embargo, cono no contamos con tanta historia solo nos enfocaremos en el único periodo completo que tenemos, año 2016.
...
df %>% 
  ...
  kbl() %>% kable_styling(full_width = F)      # acomodamos la salida en una tabla 
```

Ya con esta otra vista podemos concluir lo siguiente:

- Son en los meses de mayo y octubre donde hay mayor demanda de reservaciones.
...
df %>% 
  ...
  kbl() %>% kable_paper("hover", full_width = F) # salida en forma de tabla
```
...
df %>% 
  ...
  labs(title = "Facturación por tipo de hotel")
```
...
1. un incremento en el número de las reservaciones, hubo más clientes.
2. aumentó el precio medio.
3. crecimiento promedio de noches por reserva.
...
df %>% 
  ...
  kbl(align = "c") %>% kable_paper("hover", full_width = T)
```
&nbsp;
...
count(df, hotel, extranjero) %>% 
  ...
  kbl(align = "c") %>% kable_paper("hover", full_width = F)
```
...
La correlación resulta ser positiva de 0.54, lo que nos indica un grado de asociación moderada, sin embargo, hay que hacer el test de significancia, para confirmar que realmente existe correlación entre el número de reservas y el precio medio, ya que de lo contrario dicha sociación puede deberse al azar.
...
veamos ahora por hotel y por semana durante el periodo de análisis:
...
En las gráficas anteriores, el color de los puntos indica el número de semana, entre más claro es el color más cercano al fin de año nos encontramos. Es en hotel resort donde, en general, no se corresponde la temporada alta con precios promedio elevados, por lo que, sería conveniente analizar la posibilidad de aumentar los precios en temporada alta, o sea, buscar el precio óptimo es ese periodo del año.
...
Recordando que la curva de pareto consiste en mostrar la regla de que, aproximadamente, el 20% de las observaciones analizadas generan el 80% de sus resultados. Esta regla no es fija y puede variar la distribución, sin embargo, es un buen indicador que permite conocer a los mejores clientes, los más rentables.
...
Esta gráfica nos muestra la contribución, a nivel de país, que tienen los clientes. Ahora, sin considerar a Portugal (por ser el país en donde se encuentran los hoteles), son solo 5 países los que contribuyen a generar casi el 80% de la facturación total: Gran Bretaña, Francia, España, Alemania e Irlanda. Sabiendo esto podemos recomentar focalizar promociones y/o campañas para los turistas de esas nacionalidades.

&nbsp;

## Conclusiones

Ya habiendo terminado este proyecto de Discovery, podemos identificar las siguentes conclusiones:

* Como toda empresa que pertenece al giro del turismo sus actividades presentan temporadas de alta y baja demanda, o sea, se ven afectadas por la estacionalidad propia de su sector. En el caso de los hoteles, se detectó estacionalidad (considerando solo el año 2016 por la falta de más información histórica).
...
* El hotel resort tiene un poco más de demanda hasta el primer trimestre del año mientras que el hotel de ciudad la demanda es mayor en los meses de junio septiembre y octubre.
...
* La mayor parte de la facturación proviene del segmento parejas sin hijos; en el hotel resort equivale al 70% y en el hotel de ciudad el 65%.
